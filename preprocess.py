...
def init_crew_cast(tm_db_id):
    ...
    response = requests.get(f"{POST_BASE_URL}/{tm_db_id}/credits",
                            params={"api_key": POST_API_KEY}).json()
    ...
    if 'cast' not in response:
        ...
        for d in response['cast']:
            ...
            cast[d['original_name']] = {
                "profile_path": (IMAGE_BASE_URL + d['profile_path']) if imagePath else None,
...
def init_job(data):
    ...
    # 현재 영화의 장르 정보
    getGenres = (supabase.table("movie_genres").
                 select("genre").
    ...
    # 현재 영화의 별점 정보
    getRating = (supabase.table("Ratings_Table").
                 select("rating").
                 eq("movieId", movie_id).
                 execute().data)
    movie_rate = round(random.random() * 5, 1)
    if getRating:
        movie_rate = round(sum(rate['rating'] for rate in getRating) / len(getRating), 1)

    # 출연 배우, 감독관 정보
    tm_db_id = (supabase.table("Links_Table").
                select("tmdbId").
                eq("movieId", movie_id)
                .execute().data)[0]['tmdbId']
    movie_cast, movie_crew = init_crew_cast(tm_db_id)

    # 영화 태그 정보
    getTag = (supabase.table("Tags_Table").
              select("tag").
              eq("movieId", movie_id).
              execute().data)
...
def init_multi_thread():
    ...
    for i in range(0, total_data, 1000):
        ...
        res = (supabase.table("Movies_Table").
...
def _parse_poster(im_db_id, movieId):
    ...
    if link:
        ...
        if poster_url:
            global supabase
            supabase.table("Movies_Table").update({"poster_url": poster_url}).eq("movieId", movieId).execute()
        else:
...
def _from_api(tm_db_id, movieId):
    ...
    if not tm_db_id:
        ...
        response = requests.get(f"{POST_BASE_URL}/{tm_db_id}", params={"api_key": POST_API_KEY}).json()
...
def _parse_overview_and_story_line(im_db_id, movieId, overview):
    ...
    elements = soup.select("div.ipc-html-content-inner-div div.ipc-html-content-inner-div")
